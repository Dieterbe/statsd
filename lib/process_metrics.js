var process_metrics = function (metrics, config, ts, flushCallback) {
    ...
    for (key in counters) {
      ...
      var valuePerSecond = value / (config.flushInterval / 1000);
    ...
    for (key in timers) {
      if (timers[key].length > 0) {
        ...
        for (key2 in config.pctThreshold) {
          var pct = config.pctThreshold[key2];
          if (count > 1) {
        ...
        bins = [];
        for (var i = 0; i < config.histogram.length; i++) {
            if (key.indexOf(config.histogram[i].metric) > -1) {
                bins = config.histogram[i].bins;
                break;
            }
        }
        if(bins.length) {
